...
export const List = ({ location }: Location) => {
  ...
  useEffect(() => {
    const data = async () => {
      ...
      // if(Authorization) headers.Authorization = localStorage.getItem('token')
      const Authorization = localStorage.getItem('token') || '';
      const result = await fetch(
        'http://3.36.239.71/api/places' + window.location.search,
        {
          method: 'GET',
          headers: {
            Authorization,
  ...
  const [modal, setModal] = useState({ show: false, placeId: -1 });
  const onShowModal = (
    ...
    if (closest) return;
    else setModal({ show: true, placeId: id });
  };
              ...
              {stays &&
                stays.map((stay, idx) => (
                  <Stay
                    key={stay.id}
                    {...stay}
                    onShowModal={(e) => onShowModal(e, stay.id)}
                  />
                ))}
        ...
        <priceStateContext.Provider value={price}>
          <showModalContext.Provider value={{ modal, setModal }}>
            <personnelContext.Provider value={personnelState}>
              {modal.show && price && <Modal placeId={modal.placeId} />}
