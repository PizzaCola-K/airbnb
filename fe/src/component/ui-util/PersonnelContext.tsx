...
import { personnelAction, personnelState, personnelDispatch } from './GlobalInterface';
...
const personnelReducer = (state:typeof initPersonnelState, action: personnelAction):typeof initPersonnelState => {
    const newState:typeof initPersonnelState = state.map((v) => {return { ...v}});
    newState[action.index].count += action.counter;
    return newState;
};
    // switch (action.type) {
    //   case 'increase': 
...
export const PersonnelProvider = ({ children }:{ children: React.ReactNode }) => {
    const [personState, personDispatch] = useReducer(personnelReducer, initPersonnelState);
